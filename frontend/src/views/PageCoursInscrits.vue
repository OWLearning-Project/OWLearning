  ...
  <BContainer class="py-5">
    <BNavbar class="bg-white rounded-4 shadow-sm py-2 mb-5 justify-content-center">
      <BNavbarBrand tag="div" class="mb-0 p-0 w-100 text-center">
        <h1 class="fw-bold fs-2 m-0 mt-2" style="color: #4a2c59">Mes cours inscrits</h1>
        <p class="text-muted fs-6 mt-1 mb-2">Retrouvez les cours que vous suivez</p>
        <hr
          class="w-25 mx-auto mb-2"
          style="border-color: #a17c5b; border-width: 3px; opacity: 1"
        />
      </BNavbarBrand>
    </BNavbar>
          ...
          <BRow class="g-4">
            <BCol cols="12" md="6" lg="4" v-for="cours in coursAffiches" :key="cours.id">
              <BCard class="h-100 shadow-sm carte-cours border-0 position-relative" no-body>
                <div class="chip-progression shadow-sm fw-bold small d-flex align-items-center">
                  <span>{{ tauxProgression[cours.id] || 0 }}%</span>
                </div>
...
async function chargerCours() {
  try {
    ...
    const reponse = await axios.get('http://localhost:8080/api/cours/utilisateurs/inscriptions', {
      headers: {
...
async function chargerTauxProgression() {
  try {
    const token = localStorage.getItem('token')
    for (const cours of lesCours.value) {
      try {
        const reponse = await axios.get(`http://localhost:8080/api/progression/${cours.id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })

        tauxProgression.value[cours.id] = reponse.data.tauxProgression
      } catch (err) {
