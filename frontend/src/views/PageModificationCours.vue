...
onMounted(async () => {
  try {
    const reponse = await axios.get(`http://localhost:8080/api/cours/${idCours}`);

    titre.value = reponse.data.titre;
    description.value = reponse.data.description;
    difficulte.value = reponse.data.difficulte;
    estPrive.value = reponse.data.estPrive || false;
...
async function sauvegarderModifications() {
  ...
  try {
    ...
    await axios.put(`http://localhost:8080/api/cours/${idCours}`, donneesModifiees);
