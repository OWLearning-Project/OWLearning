...
import app.OwLearning.Services.Interfaces.IServiceProgression;
...
public class ProgressionController {
    ...
    @GetMapping("/{idCours}")
    public ResponseEntity<?> getTauxProgression(@PathVariable("idCours") int coursId, @AuthenticationPrincipal UtilisateurAuthentifieRequest utilisateurAuthentifieDTO){
        if (coursId <= 0 )
        {
